...
internal class QuizControllerTest {
    ...
    fun shouldAnswerCorrect() {
        ...
        assertThat(result.participants[0]).isEqualTo(ParticipantDTO(UUID.randomUUID(), "Sandra", false, 1))
        assertThat(result.participants[1]).isEqualTo(ParticipantDTO(UUID.randomUUID(), "Erik", false, 0))
        assertThat(result.playedQuestions).isEmpty()
        assertThat(result.openQuestions).isEqualTo(listOf(QuestionDTO(UUID.randomUUID(), "Warum ist die Banane krumm?", false)))
        assertThat(result.links).hasSize(4)
        assertThat(result.getLink("createParticipant"))
                .map { it.href }
    ...
    fun shouldRepopenQuestion() {
        ...
        assertThat(result.participants[1]).isEqualTo(ParticipantDTO(UUID.randomUUID(), "Erik", false, 0))
        assertThat(result.playedQuestions).isEmpty()
        assertThat(result.openQuestions).isEqualTo(listOf(QuestionDTO(UUID.randomUUID(), "Warum ist die Banane krumm?", true)))
        assertThat(result.links).hasSize(4)
        assertThat(result.getLink("createParticipant"))
                .map { it.href }
                .contains("/api/quiz/14/participants")
