...
class DefaultQuizService(private val eventRepository: EventRepository,
    ...
    override fun createQuestion(command: CreateQuestionCommand): Mono<Unit> {
        ...
        return eventRepository.determineEvents(command.quizId)
                ...
                .flatMap { eventRepository.storeEvent(QuestionCreatedEvent(command.quizId, command.question, sequenceNumber = it.sequenceNumber + 1)) }
                .map {
    ...
    override fun editQuestion(command: EditQuestionCommand): Mono<Unit> {
        ...
        return eventRepository.determineEvents(command.quizId)
                ...
                .filter { !it.finished }
                .switchIfEmpty(Mono.error(QuizFinishedException()))
                .flatMap { eventRepository.storeEvent(QuestionEditedEvent(command.quizId, command.question, sequenceNumber = it.sequenceNumber + 1)) }
                .map {
                    undoneEventsCache.remove(it.quizId)
                    eventBus.post(it)
                }
    ...
    override fun createParticipant(command: CreateParticipantCommand): Mono<Unit> {
        ...
        return eventRepository.determineEvents(command.quizId)
                ...
                .flatMap { eventRepository.storeEvent(ParticipantCreatedEvent(command.quizId, command.participant, sequenceNumber = it.sequenceNumber + 1)) }
                .map {
                    undoneEventsCache.remove(it.quizId)
                    eventBus.post(it)
                }
    ...
    override fun deleteParticipant(command: DeleteParticipantCommand): Mono<Unit> {
        ...
        return eventRepository.determineEvents(command.quizId)
                ...
                .flatMap { eventRepository.storeEvent(ParticipantDeletedEvent(command.quizId, command.participantId, sequenceNumber = it.sequenceNumber + 1)) }
                .map {
                    undoneEventsCache.remove(it.quizId)
                    eventBus.post(it)
                }
    ...
    override fun deleteQuestion(command: DeleteQuestionCommand): Mono<Unit> {
        ...
        return eventRepository.determineEvents(command.quizId)
                ...
                .filter { !it.finished }
                .switchIfEmpty(Mono.error(QuizFinishedException()))
                .flatMap { eventRepository.storeEvent(QuestionDeletedEvent(command.quizId, command.questionId, sequenceNumber = it.sequenceNumber + 1)) }
                .map {
                    undoneEventsCache.remove(it.quizId)
                    eventBus.post(it)
                }
    ...
    override fun buzzer(command: BuzzerCommand): Mono<Unit> {
        ...
        return eventRepository.determineEvents(command.quizId)
                ...
                .flatMap { eventRepository.storeEvent(BuzzeredEvent(command.quizId, command.participantId, sequenceNumber = it.sequenceNumber + 1)) }
                .map {
                    undoneEventsCache.remove(it.quizId)
                    eventBus.post(it)
                }
    ...
    override fun toggleAnswerRevealAllowed(command: ToggleAnswerRevealAllowedCommand): Mono<Unit> {
        ...
        return eventRepository.determineEvents(command.quizId)
                ...
                .switchIfEmpty(Mono.error(QuizFinishedException()))
                .filter { it.hasParticipantWithId(command.participantId) }
                .flatMap { eventRepository.storeEvent(ToggleAnswerRevealAllowedEvent(command.quizId, command.participantId, sequenceNumber = it.sequenceNumber + 1)) }
                .map {
                    undoneEventsCache.remove(it.quizId)
                    eventBus.post(it)
                }
    ...
    override fun startNewQuestion(command: AskQuestionCommand): Mono<Unit> {
        ...
        return eventRepository.determineEvents(command.quizId)
                ...
                .filter { !it.finished }
                .switchIfEmpty(Mono.error(QuizFinishedException()))
                .flatMap { eventRepository.storeEvent(QuestionAskedEvent(command.quizId, command.questionId, sequenceNumber = it.sequenceNumber + 1)) }
                .map { eventBus.post(it) }
    ...
    override fun rate(command: AnswerCommand): Mono<Unit> {
        ...
        return eventRepository.determineEvents(command.quizId)
                ...
                .filter { !it.finished }
                .switchIfEmpty(Mono.error(QuizFinishedException()))
                .flatMap { eventRepository.storeEvent(AnsweredEvent(command.quizId, command.participantId, command.answer, sequenceNumber = it.sequenceNumber + 1)) }
                .map {
                    undoneEventsCache.remove(it.quizId)
                    eventBus.post(it)
                }
    ...
    override fun reopenQuestion(command: ReopenCurrentQuestionCommand): Mono<Unit> {
        ...
        return eventRepository.determineEvents(command.quizId)
                ...
                .filter { !it.finished }
                .switchIfEmpty(Mono.error(QuizFinishedException()))
                .flatMap { eventRepository.storeEvent(CurrentQuestionReopenedEvent(command.quizId, sequenceNumber = it.sequenceNumber + 1)) }
                .map { eventBus.post(it) }
                .flatMapMany { eventRepository.determineEvents(command.quizId) }
                .reduce(Quiz(name = "")) { quiz: Quiz, event: Event -> event.process(quiz) }
                .map {
    ...
    private fun timeDecreased(quizId: UUID, questionId: UUID): Mono<Unit> {
        return Mono.just(Unit)
            .map { eventBus.post(TimeToAnswerDecreasedEvent(quizId, questionId, sequenceNumber = Long.MAX_VALUE)) }
    }
    ...
    override fun revealAnswers(command: RevealAnswersCommand): Mono<Unit> {
        ...
        return eventRepository.determineEvents(command.quizId)
                ...
                .filter { !it.finished }
                .switchIfEmpty(Mono.error(QuizFinishedException()))
                .flatMap { eventRepository.storeEvent(AnswersRevealedEvent(command.quizId, sequenceNumber = it.sequenceNumber + 1)) }
                .map {
    ...
    override fun finishQuiz(command: FinishQuizCommand): Mono<Unit> {
        ...
        return eventRepository.determineEvents(command.quizId)
            ...
            .filter { !it.finished }
            .switchIfEmpty(Mono.error(QuizFinishedException()))
            .flatMap { eventRepository.storeEvent(QuizFinishedEvent(command.quizId, sequenceNumber = it.sequenceNumber + 1)) }
            .map {
                undoneEventsCache.remove(it.quizId)
                eventBus.post(it)
            }
